...
export default function Live() {
  ...
  return (
    <section className="p-8 flex flex-col gap-2">
      {
        logs.map(log => (
          <article className="bg-[#FAFAFA] p-4 rounded-lg flex gap-4 items-center">
            <DocumentIcon width={20} height={20} color="#86898D" />
            <p className="text-sm w-[20%] text-[#86898D]">{new Date(log.timestamp).toISOString()}</p>
            <p className="text-sm w-[10%] line-clamp-1">{log.channel}</p>
            <p className="text-sm w-[5%] text-[#1C65F4]">{log.level}</p>
            <p className="text-sm w-[30%] text-[#1E1E1E] line-clamp-1 font-light">{log.message}</p>
            <p className="text-sm w-[30%] text-[#5D5D5D] line-clamp-1 font-light">{JSON.stringify(log.data)}</p>
          </article> 
        ))
      }
    </section>
